  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      for(proc1=ptable.proc;proc1<&ptable.proc[NPROC];proc1++){
	     ...
	     continue;
          if(highProc->priority > proc1->priority) //현재 값이 작다면 priority값을 높여줌
	     highProc=proc1;
      }
      p=highProc;
      c->proc = p;
...
{
	struct proc *p;
	sti(); //이 processor에set interrupt
	acquire(&ptable.lock);
	cprintf("PID \t STATE \t \t NAME \t NICE\n");
	for(p=ptable.proc; p<&ptable.proc[NPROC]; p++){
	    if(p->state==SLEEPING) //현재 process가 SLEEPING 상태 일때
	       cprintf("%d \t SLEEPING \t %s \t %d\n", p->pid, p->name, p->nice);
            else if(p->state==RUNNING) //현재 process가 RUNNING 상태 일때
               cprintf("%d \t RUNNING \t %s \t %d\n", p->pid, p->name, p->nice);
            if(p->state==RUNNABLE) //현재 process가 RUNNABLE 상태 일때
